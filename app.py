...
class AdvancedLumpSumAnalyzer:
    ...
    def phase2_forecast_funds(self):
        ...
        for fund in self.selected_funds:
            try:
                ...
                if len(nav_series) < 252:
                    # Create dummy forecast_df for plotting
                    last_date = nav_series.index[-1] if len(nav_series) > 0 else pd.Timestamp.now()
                    future_dates = pd.date_range(last_date, periods=self.forecast_weeks+1, freq='W')[1:]
                    dummy_forecast = pd.DataFrame({'predicted_cagr': [0.0] * len(future_dates)}, index=future_dates)
                    ...
                    self.model_selection_info[fund] = {
                        ...
                        'characteristics': {'volatility_category': 'N/A', 'behavior_pattern': 'N/A'},
                ...
                self.model_selection_info[fund] = {
                    ...
                    'forecast_df': best['forecast_df']  # 👈 Always stored
                }
                ...
                # Create dummy forecast_df for failed funds
                last_date = self.nav_data.index[-1] if len(self.nav_data) > 0 else pd.Timestamp.now()
                future_dates = pd.date_range(last_date, periods=self.forecast_weeks+1, freq='W')[1:]
                dummy_forecast = pd.DataFrame({'predicted_cagr': [0.0] * len(future_dates)}, index=future_dates)
                ...
                self.model_selection_info[fund] = {
                    ...
                    'rmse': 0.0,
                    'direction_accuracy': 0.0,
                    'characteristics': {'volatility_category': 'Error', 'behavior_pattern': 'Error'},
                    'forecast_df': dummy_forecast
                }
                self.cagr_forecasts[fund] = 0.0
                st.error(f"⚠️ Forecasting failed for {fund}: {str(e)}")
    ...
    def plot_fund_vs_benchmark_forecast(self, fund_id):
        ...
        try:
            ...
            vol = char.get('volatility_category', 'N/A')
            beh = char.get('behavior_pattern', 'N/A')
            title = f"{self.fund_mapping[fund_id]} ({fund_id}) vs {bench}\nModel: {selected_model} | Volatility: {vol} | Behavior: {beh}"
            ...
            # Add warning overlay if forecast is dummy/error
            if selected_model in ['N/A', 'Error']:
                ax.text(0.5, 0.5, '⚠️ Forecast Unavailable\n(Insufficient Data or Error)',
